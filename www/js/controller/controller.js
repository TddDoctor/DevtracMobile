...
//        localStorage.pnid = fObject['field_fieldtrip_places']['und'][0]['target_id'];
    ...
    buildSelect: function (vocabularyname, selectedoptions) {
      ...
      devtrac.indexedDB.open(function (db) {
        devtrac.indexedDB.getAllTaxonomyItems(db, vocabularyname, function (taxonomies) {
          ...
          for(var x in taxonomies) {
            optgroup = optgroup + "<optgroup label=" + taxonomies[x]['hname'] + ">";
            if(taxonomies[x]['children'] != undefined) {
              optgroup =  optgroup + controller.addoptions(taxonomies[x]['children'], '', '');
            }
          }
    ...
    addoptions: function(optionchildren, options, delimeter) {
      ...
      for(var y in optionchildren){
        ...
        if(optionchildren[y]["children"] != undefined) {
          options = options + controller.addoptions(optionchildren[y]["children"], options, delimeter);
        }
      }
